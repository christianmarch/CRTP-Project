...
struct Task
{
    ...
    int activeCount;        // numb of active instances of this task
    unsigned int dec;       // variable decremented during execution
};

// Global variables
struct Task tasks[] = {
    {"0", 8, 3, 8, 95, 0, 2100000000},
    {"1", 14, 4, 14,50, 0, 2800000000},
    {"2", 22, 5, 22, 25, 0, 3700000000},
    {"3", 6, 4, 6, 99, 0, 2800000000},
    //{"4", 5, 1,, 0, 300000, 0},
    //{"5", 10, 4,, 0, 400000, 0}
};
...
static int time_analysis(struct Task *task)
{
    ...
    for (int i = 0; i < num_tasks; ++i)
    {
        ...
        int tmp = tasks[i].activeCount;
        if(strcmp(tasks[i].name,task->name) == 0) tmp++;
        for(int n=0; n<tmp; n++)
        {
            // Evaluation of the task i
            printf("\nEvaluation of task %s.%d\n", tasks[i].name,n);
            k = 0;
            ...
            while(w_curr <= tasks[i].deadline)
            {
                ...
                for(int j = 0; j < num_tasks; ++j)
                {
                    ...
                    {
                        for(int m=0; m<tasks[j].activeCount; m++)
                        {
            ...
            printf("FINAL w_curr=Ri for task %s.%d is %d with deadline %d\n", tasks[i].name, n, w_curr, tasks[i].deadline);
...
static void handleConnection(int currSd)
{
    ...
    for (;;)
    {
        ...
        else if (strncmp(command, "rm_task", 7) == 0 && len < 10)
        {
            // Bind the command with the matching task in the struct
            struct Task *task = NULL;
            for (int i = 0; i < num_tasks; ++i)
            {
                if (strcmp(tasks[i].name, &command[7]) == 0)
                {
                    task = &tasks[i];
                    break;
                }
        ...
        else if (strncmp(command, "task", 4) == 0 && len < 7)
        {
        	...
        	// Perform the time analysis and if it returns true then create a new thread
            // If it returns false then the task is not schedulable
            if(time_analysis(task)){
                ...
                if (pthread_create(&thread, &attr, &task_routine, (void *)task) != 0)
